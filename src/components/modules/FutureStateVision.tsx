...
const FutureStateVision: React.FC = () => {
  const { t } = useTheme();
  const [sliders, setSliders] = useState<Record<string, number>>({ secModel: 7, netModel: 6, branchCtrl: 5, cloudAdj: 8, zeroTrust: 9, observ: 7, auto: 8, resil: 7, aiEdge: 6, supportModel: 7 });
  const [posture, setPosture] = useState('cloud_accel');
  const postures = [
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <GlassCard className="animate-fade-up delay-1">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: 10 }}>
          {postures.map(p => (
            <div key={p.id} onClick={() => setPosture(p.id)} style={{ padding: '18px', borderRadius: t.r.lg, cursor: 'pointer', transition: 'all 0.3s', background: posture === p.id ? p.color + '12' : t.bgGlass, border: `1px solid ${posture === p.id ? p.color + '50' : t.borderSubtle}`, boxShadow: posture === p.id ? `0 0 20px ${p.color}12` : 'none' }}>
      ...
      <GlassCard className="animate-fade-up delay-2">
        <SectionHeader tag="PREFERENCES" sub="Configure capability targets">Architecture Sliders</SectionHeader>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 28px' }}>
          {defs.map(s => <SliderControl key={s.key} label={s.label} desc={s.desc} value={sliders[s.key]} onChange={v => setSliders(p => ({ ...p, [s.key]: v }))} color={sliders[s.key] >= 7 ? t.emerald : t.accent} />)}
        </div>
      </GlassCard>
